...
2. `Bilgisayar_Araci.ps1` dosyasına **sağ tık → "PowerShell ile çalıştır"**.
...
Windows PowerShell 5.1, BOM içermeyen `.ps1` dosyalarını UTF-8 yerine sistem kod sayfasına göre okur; bu yüzden Türkçe karakterler ve çerçeve simgeleri (╔ ║ ✦ vb.) yerelde bozuk görünebilir. `irm | iex` (Yöntem 1) ile çalıştırırken bu sorun yaşanmaz; yalnızca yerel dosyayı çalıştırırken geçerlidir.

1. İndirilen **zip dosyasını** Masaüstüne çıkarın (klasör adı: `bilgisayar-araci-main`).
2. Bir PowerShell penceresinde şu komutu çalıştırın:
