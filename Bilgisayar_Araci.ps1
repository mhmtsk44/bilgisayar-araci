...
if (-not (Test-Admin)) {
    ...
    try {
        if ($bk.Tip -eq "Dosya") {
            # -NoExit YOK -> wt acildiktan sonra bu ara pencere kapanir
            Start-Process powershell -ArgumentList "-ExecutionPolicy Bypass -File `"$($bk.Deger)`"" -Verb RunAs
        } else {
            Start-Process powershell -ArgumentList "-ExecutionPolicy Bypass -Command `"$($bk.Deger)`"" -Verb RunAs
        }
    } catch {
