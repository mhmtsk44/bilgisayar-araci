...
function Get-BaslatmaKomutu {
    if (-not [string]::IsNullOrWhiteSpace($BetikYolu) -and (Test-Path $BetikYolu)) {
        ...
        # YEDEK: yerel dosya yoksa uzaktan TEMP'e indir ve oradan çalıştır (BOM hatasını engeller)
        $tmpYol = Join-Path $env:TEMP "Bilgisayar_Araci_Web.ps1"
        return @{ Tip = "Komut"; Deger = "irm '$ScriptUrl' -OutFile '$tmpYol'; & '$tmpYol'" }
    }
}
