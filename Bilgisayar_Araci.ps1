...
# ===== ÜST BAŞLIK BANDI =====
    ...
    # 1. Ana Başlık (Modern ikon ve normalleştirilmiş harf boşlukları)
    $baslik = "💻 BİLGİSAYAR YÖNETİM ARACI"
    ...
    # 2. İç Ayraç (Başlık ile slogan arasına kutu genişliğine uyumlu çizgi)
    $ayrac = "─" * ($BoxWidth - 8) # Kenarlardan 4'er boşluk bırakır
    $aPad = [math]::Max(1, [math]::Floor(($BoxWidth - $ayrac.Length) / 2))
    Write-Host "║" -ForegroundColor $Tema.Cerceve -NoNewline
    Write-Host ((" " * $aPad) + $ayrac + (" " * ($BoxWidth - $ayrac.Length - $aPad))) -ForegroundColor $Tema.Soluk -NoNewline
    Write-Host "║" -ForegroundColor $Tema.Cerceve

    # 3. Alt Slogan (Noktalar yerine elmas sembolü ve daha ferah boşluklar)
...
# ===== CANLI MİNİ SİSTEM DURUMU =====
    ...
    try {
        ...
        # RAM Hesabı (Fiziksel bellek modüllerinden donanım kesintisi olmadan net okuma)
        $ramModulleri = Get-CimInstance Win32_PhysicalMemory -ErrorAction SilentlyContinue
        $ramTop = [math]::Round((($ramModulleri | Measure-Object -Property Capacity -Sum).Sum) / 1GB)
