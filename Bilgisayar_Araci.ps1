...
function Assert-WinGetModulu {
    ...
    Write-Host "  Gerekli PowerShell modülü (Microsoft.WinGet.Client) kuruluyor..." -ForegroundColor Yellow
    Write-Host "  (Bu, yalnızca ilk kullanımda bir kez yapılır, lütfen bekleyin...)" -ForegroundColor $Tema.Soluk
    
    try {
        # 1. Yabancı PC'lerde bağlantı hatasını engellemek için TLS 1.2'yi zorla
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12

        # 2. Arka plan gereksinimi olan NuGet'i sessizce kur
        Install-PackageProvider -Name NuGet -Force -Scope CurrentUser -ErrorAction SilentlyContinue | Out-Null
        
        # 3. Y/N onay sorusunda programın donmasını engellemek için depoyu güvenilir işaretle
        Set-PSRepository -Name "PSGallery" -InstallationPolicy Trusted -ErrorAction SilentlyContinue

        # 4. Modülü indir ve kur (-AllowClobber eklenerek olası çakışmalar ezilir)
...
function Update-AllApps {
    ...
    foreach ($u in $secilenler) {
        Write-Host ("  ▸ " + $u.Ad + " güncelleniyor...") -ForegroundColor Yellow
        try {
...
function Clean-Logs {
    ...
    try {
        ...
        if ($zamanAsimi -gt 0) {
            Write-Host "  Not: $zamanAsimi günlük zaman aşımına uğradığı için atlandı." -ForegroundColor $Tema.Soluk
        }
...
function Clean-RecycleBin {
    ...
    try {
        # Doğrudan Windows Shell'e bağlanıp sepetin içindeki dosya sayısını okuyoruz
        $kutu = (New-Object -ComObject Shell.Application).NameSpace(10)
        $dosyaSayisi = $kutu.Items().Count

        if ($dosyaSayisi -eq 0) {
            Write-Host "  ✓  Geri dönüşüm kutusu zaten boş." -ForegroundColor $Tema.Basari
        } else {
            Clear-RecycleBin -Force -ErrorAction Stop
            Write-Host "  ✓  Geri dönüşüm kutusu başarıyla temizlendi ($dosyaSayisi öğe silindi)." -ForegroundColor $Tema.Basari
        }
    }
    catch {
        Write-Host "  ✗  Geri dönüşüm kutusu işleminde hata: $($_.Exception.Message)" -ForegroundColor $Tema.Hata
    }
...
function Clean-Disk {
    ...
    Write-Host "  0) İptal" -ForegroundColor $Tema.Soluk
    Write-Host ""
    ...
    try {
        if ($sec -eq "1") {
            ...
            # /autoclean varsayılan güvenli dosyaları kullanıcıya sormadan temizler
            ...
            # /d c: doğrudan C sürücüsü için arayüzü getirir
            Start-Process cleanmgr -ArgumentList "/d c:" -Wait
            Write-Result $true "Disk Temizleme aracı kapatıldı."
        } elseif ($sec -eq "0" -or [string]::IsNullOrWhiteSpace($sec)) {
            Write-Result $false "İşlem iptal edildi."
        } else {
            Write-Result $false "Geçersiz seçim, işlem iptal edildi."
        }
    } catch {
...
function Search-App {
    ...
    $sonucRaw = winget search $arama 2>&1 | Out-String
    $satirlarWinget = $sonucRaw -split "`r?`n"

    # ===== Ayraç satırını bul (dil bağımsız); üstündeki satır başlıktır =====
    $sepIndex = -1
    for ($i = 0; $i -lt $satirlarWinget.Count; $i++) {
        if ($satirlarWinget[$i] -match '^-{5,}\s*$') { $sepIndex = $i; break }
    }

    $sonucSatirlari = @()

    if ($sepIndex -ge 1) {
        $header = $satirlarWinget[$sepIndex - 1]
        ...
        for ($i = $sepIndex + 1; $i -lt $satirlarWinget.Count; $i++) {
            $satir = $satirlarWinget[$i]
    ...
    if ($sonucSatirlari.Count -eq 0) {
        ...
        # Kalan alanın tamamı Ad (isim) sütununa ayrılır (5 sütun arası 4 çift-boşluk + baştaki 2)
        $adGen = $konsolGenislik - $idGen - $surumGen - $eslesmeGen - $kaynakGen - 10
        if ($adGen -lt 12) { $adGen = 12 }

        $baslikSatiri = "  " + "Ad".PadRight($adGen) + "  " + "Id".PadRight($idGen) + "  " + "Sürüm".PadRight($surumGen) + "  " + "Eşleşme".PadRight($eslesmeGen) + "  " + "Kaynak"
        Write-Host $baslikSatiri -ForegroundColor $Tema.Baslik
        Write-Host ("  " + ("─" * [math]::Min($konsolGenislik - 2, ($adGen + $idGen + $surumGen + $eslesmeGen + $kaynakGen + 10)))) -ForegroundColor $Tema.Soluk
...
function Show-Help {
    ...
    Write-Host "  Güncelleme : 06.07.2026" -ForegroundColor $Tema.Metin
    Write-Host ""
...
if ($WingetVar -and -not (Get-Module -ListAvailable -Name Microsoft.WinGet.Client)) {
    Write-Host "  Gerekli yönetim modülleri arka planda hazırlanıyor, lütfen bekleyin..." -ForegroundColor DarkGray
