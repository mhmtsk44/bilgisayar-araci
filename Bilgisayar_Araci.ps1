...
function Repair-Disk {
    ...
    switch ($anaSecim) {
        ...
        1 {
            Write-Host ""
            Write-Host "  SFC taraması başlatılıyor..." -ForegroundColor $Tema.Vurgu
        ...
        2 {
            Write-Host ""
            Write-Host "  DISM onarımı başlatılıyor..." -ForegroundColor $Tema.Vurgu
            $dismKod = Invoke-YuzdeliIslem -Dosya "DISM.exe" -Argumanlar "/Online /Cleanup-Image /RestoreHealth" -NullTemizle
            if ($dismKod -eq 0) {
        ...
        4 {
            Write-Host ""
            Write-Host "  [1/2] DISM onarımı çalışıyor..." -ForegroundColor $Tema.Vurgu
            $dismKod = Invoke-YuzdeliIslem -Dosya "DISM.exe" -Argumanlar "/Online /Cleanup-Image /RestoreHealth" -NullTemizle
            if ($dismKod -eq 0) {
                Write-Result $true "DISM onarımı tamamlandı."
            } else {
            ...
            Write-Host ""
            Write-Host "  [2/2] SFC taraması çalışıyor..." -ForegroundColor $Tema.Vurgu
            $sfcKod = Invoke-YuzdeliIslem -Dosya "sfc.exe" -Argumanlar "/scannow" -NullTemizle
            if ($sfcKod -eq 0) {
                Write-Result $true "SFC taraması tamamlandı."
            } else {
    ...
    while ($true) {
        ...
        if ($sec -eq "H" -or $sec -eq "h" -or $sec -eq "T" -or $sec -eq "t" -or $sec -match "[0-9]") {
            ...
            if ($wingetGerekli -and -not $WingetVar) {
                ...
                Write-Host ""
                Write-Host "  Winget'i kurmak için ana menü > 26) Yardım bölümünü kullanın" -ForegroundColor Yellow
...
$Menu = @(
    ...
    @{ No = 17; Grup = "BAKIM";     Ad = "Ağ Ayarlarını Sıfırla";              Eylem = { Reset-Network } }
    @{ No = 18; Grup = "BAKIM";     Ad = "Geri Yükleme Noktası Oluştur";       Eylem = { New-RestorePoint } }
    @{ No = 19; Grup = "BAKIM";     Ad = "Yazıcı Kuyruğunu Temizle";           Eylem = { Clear-PrintQueue } }

    @{ No = 20; Grup = "BİLGİ";     Ad = "Sistem Bilgileri";                   Eylem = { Show-SystemInfo } }
    @{ No = 21; Grup = "BİLGİ";     Ad = "Disk Özeti";                         Eylem = { Show-DiskSummary } }
    @{ No = 22; Grup = "BİLGİ";     Ad = "Disk Sağlığı (SMART)";               Eylem = { Show-DiskHealth } }
    @{ No = 23; Grup = "BİLGİ";     Ad = "Başlangıç Programları";              Eylem = { Show-Startup } }
    @{ No = 24; Grup = "BİLGİ";     Ad = "Sistem Sağlık Özeti";                Eylem = { Show-HealthSummary } }

    @{ No = 25; Grup = "DİĞER";     Ad = "Yönetim Klasörleri Oluştur";         Eylem = { New-AdminFolders } }
    @{ No = 26; Grup = "DİĞER";     Ad = "Yardım / Hakkında";                  Eylem = { Show-Help } }
)
