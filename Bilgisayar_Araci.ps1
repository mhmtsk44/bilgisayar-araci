...
function Clean-Temp {
        ...
        foreach ($tarayici in $tarayicilar) {
            if (Test-Path $tarayici.Yol) {
                ...
                if ($profiller) {
                    ...
                    if ($surecler) {
                        Write-Host ""
                        Write-Host ("  ⚠ $($tarayici.Ad) açık durumda.") -ForegroundColor Yellow
				...
				Start-Sleep -Seconds 2
                            ...
                            catch { $kapatildi = $false; Write-Host ("  ⚠ $($tarayici.Ad) kapatılamadı, atlandı.") -ForegroundColor Yellow }
    ...
    if ($secilenKategoriler -contains 3) {
        ...
        if ((Test-Path $updateYolu) -and (Test-GuvenliYol $updateYolu)) {
            ...
            $dosyalar = Get-ChildItem -Path $updateYolu -Recurse -Force -File -ErrorAction SilentlyContinue
            foreach ($d in $dosyalar) {
                try {
                    $boyutMB = $d.Length / 1MB
                    Remove-Item -LiteralPath $d.FullName -Force -ErrorAction Stop
            ...
            Get-ChildItem -Path $updateYolu -Recurse -Force -Directory -ErrorAction SilentlyContinue |
                Sort-Object FullName -Descending |
                ForEach-Object { try { Remove-Item -LiteralPath $_.FullName -Force -Recurse -ErrorAction Stop } catch {} }

            if ($hedefSilinen -gt 0) {
                $hedefKazancYuvarli = [math]::Round($hedefKazanc, 2)
                Write-Host ("  ✓ " + "Windows Update".PadRight(22) + " temizlendi — $hedefSilinen dosya, $hedefKazancYuvarli MB") -ForegroundColor $Tema.Basari
                $toplamKazanc  += $hedefKazanc
                $toplamSilinen += $hedefSilinen
            }
            $toplamHata += $hedefHata
    ...
    if ($secilenKategoriler -contains 5) {
        try {
            ...
            if ($kutu.Items().Count -gt 0) {
                Clear-RecycleBin -Force -ErrorAction Stop
                Write-Host ("  ✓ " + "Çöp Kutusu".PadRight(22) + " temizlendi.") -ForegroundColor $Tema.Basari
            }
        } catch { $toplamHata++ }
    }
        ...
        try {
            ...
            if ($loglar.Count -gt 0) {
                ...
                foreach ($log in $loglar) {
                    ...
                    try { $proc.Start() | Out-Null; if ($proc.WaitForExit(3000) -and $proc.ExitCode -eq 0) { $logBasarili++ } } catch {}
                }
...
function Show-Help {
    ...
    Write-Host "  Güncelleme : 07.07.2026" -ForegroundColor $Tema.Metin
    Write-Host ""
    ...
    while ($true) {
        ...
        if ($sec -eq "H" -or $sec -eq "h" -or $sec -eq "T" -or $sec -eq "t" -or $sec -match "[0-9]") {
            ...
            if ($wingetGerekli -and -not $WingetVar) {
                ...
                Write-Host ""
                Write-Host "  Winget'i kurmak için ana menü > 22) Yardım bölümünü kullanın" -ForegroundColor Yellow
