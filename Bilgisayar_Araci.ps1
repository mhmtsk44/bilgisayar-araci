...
# ===================== SÜRÜM KONTROLÜ (Windows 10 Altı Sürüm Engeli) =====================
...
if ($osMajor -lt 10) {
    ...
    Write-Host "  ║                                                                        ║" -ForegroundColor $Tema.Cerceve
    Write-Host "  ║             ⚠ SİSTEM SÜRÜMÜ DESTEKLENMİYOR (UYARI)                     ║" -ForegroundColor $Tema.Uyari
    Write-Host "  ║                                                                        ║" -ForegroundColor $Tema.Cerceve
    Write-Host "  ╚════════════════════════════════════════════════════════════════════════╝" -ForegroundColor $Tema.Cerceve
    Write-Host ""
    Write-Host "    Bu araç, Winget ve modern altyapı bileşenlerini kullandığı" -ForegroundColor $Tema.Metin
    Write-Host "    için yalnızca Windows 10 ve Windows 11 üzerinde çalışmaktadır." -ForegroundColor $Tema.Metin
    Write-Host ""
    Write-Host "    Mevcut sisteminizin Windows 10'dan daha eski olduğu tespit edildi." -ForegroundColor $Tema.Soluk
    Write-Host "    İşlem güvenliğiniz için başlatma durduruldu." -ForegroundColor $Tema.Hata
    Write-Host ""
...
$script:LogMaksimumBoyutMB = 1   # Bu boyutu aşınca eski log .old olarak saklanır, yenisi sıfırdan başlar
...
function Yaz-Log {
    ...
    try {
        # NOT: Log dosyası her çalıştırmada sonuna ekleniyordu ve hiç sınırlandırılmıyordu;
        # bu araç uzun süre / sık kullanıldıkça winget-kurulum.log sınırsız büyürdü.
        # Burada boyut belirli bir eşiği geçtiğinde eski log ".old" uzantısıyla bir kez
        # yedeklenip (bir önceki .old varsa üzerine yazılır) yeni log sıfırdan başlıyor;
        # böylece diskte en fazla ~2 katı (mevcut + .old) yer kaplıyor.
        if (Test-Path $script:LogDosyasi) {
            $mevcutBoyut = (Get-Item $script:LogDosyasi -ErrorAction SilentlyContinue).Length
            if ($mevcutBoyut -gt ($script:LogMaksimumBoyutMB * 1MB)) {
                $eskiLog = "$script:LogDosyasi.old"
                Move-Item -Path $script:LogDosyasi -Destination $eskiLog -Force -ErrorAction SilentlyContinue
            }
        }
        $satir = "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')  [$Seviye]  $Mesaj"
        $satir | Out-File -FilePath $script:LogDosyasi -Append -Encoding UTF8
    } catch {}
}
...
# ===================== DOSYA İNDİRME YARDIMCISI (Asenkron + Animasyonlu + Korumalı) =====================
function Indir-Dosya {
    ...
    $deneme = 0
    $basarili = $false
    # Modern yükleme animasyonu (Braille karakterleri)
    $spinnerKarakterleri = @("⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏")
    while ($deneme -lt $MaksimumDeneme -and -not $basarili) {
        $deneme++
        $job = $null
        try {
            # 1. İndirme işlemini arka planda (Job) başlat
            $job = Start-Job -ScriptBlock {
                param($u, $h, $t)
                $ErrorActionPreference = 'Stop'
                # NOT: Job ayrı bir process'te çalıştığı için ana konsoldaki
                # $ProgressPreference ayarı buraya miras kalmıyor. Bunu açıkça
                # 'SilentlyContinue' yapmazsak Invoke-WebRequest kendi ilerleme
                # kaydını job akışına yazabilir; gereksiz yük ve gürültüyü önlemek
                # için burada da sessizleştiriyoruz.
                $ProgressPreference = 'SilentlyContinue'
                Invoke-WebRequest -Uri $u -OutFile $h -UseBasicParsing -TimeoutSec $t
            } -ArgumentList $Url, $Hedef, $Timeout
            $c = 0
            # 2. Arka plan işi sürdüğü müddetçe ekranda animasyonu döndür
            while ($job.State -eq 'Running') {
                $karakter = $spinnerKarakterleri[$c % $spinnerKarakterleri.Count]
                # `r (Carriage Return) ile aynı satırın başına dönüp metni üst üste yazıyoruz (spam engellenir)
                Write-Host "`r        $karakter İndiriliyor (Deneme: $deneme/$MaksimumDeneme)... " -NoNewline -ForegroundColor $Tema.Uyari
                $c++
                Start-Sleep -Milliseconds 100
            }
            # Animasyon bitince satırı temizle
            Write-Host "`r                                                                      `r" -NoNewline
            # 3. Job'un sonucunu al (hata varsa catch bloğuna düşürür)
            Receive-Job -Job $job -ErrorAction Stop | Out-Null
            if ($job.State -eq 'Completed' -and (Test-Path $Hedef)) {
                Yaz-Log "Indirme basarili: $Url (Deneme: $deneme)"
                $basarili = $true
            } else {
                throw "İndirme job'ı tamamlanamadı veya dosya oluşturulamadı."
            }
        } catch {
            Write-Host "`r                                                                      `r" -NoNewline
            $hataMesaji = $_.Exception.Message
            if ($deneme -lt $MaksimumDeneme) {
                Write-Host "        [$deneme/$MaksimumDeneme] Ağ dalgalanması: ($SaniyeBekle sn sonra tekrar denenecek)" -ForegroundColor $Tema.UyariSoluk
            ...
            # Kalan iş kalıntılarını bellekten temizle
            if ($null -ne $job) {
...
function Install-WingetManuel {
    ...
    try {
        ...
        # 1. URL'leri Hazırla (GitHub API'den linkleri çekmek hızlıdır)
        Write-Host "        İndirme adresleri çözümleniyor..." -ForegroundColor $Tema.Soluk
        $appUrl = "https://aka.ms/getwinget"
        $licUrl = $null
        try {
            $rel = Invoke-RestMethod -Uri "https://api.github.com/repos/microsoft/winget-cli/releases/latest" -UseBasicParsing -TimeoutSec 15 -ErrorAction Stop
            ...
            if ($msix) { $appUrl = $msix.browser_download_url }
            if ($lic)  { $licUrl = $lic.browser_download_url }
        } catch { Yaz-Log "GitHub Release alinamadi, yedek baglanti kullanilacak." 'UYARI' }

        # 2. Paralel İndirme İşlerini (Jobs) Başlat
        # Arka planda çalışacak izole, kendi dalgalanma korumasına sahip indirme betiği:
        $indirmeJobScript = {
            param($u, $h)
            $ProgressPreference = 'SilentlyContinue'
            $ErrorActionPreference = 'Stop'
            ...
            for ($i = 1; $i -le 3; $i++) {
                try {
                    Invoke-WebRequest -Uri $u -OutFile $h -UseBasicParsing -TimeoutSec 120
                    $basarili = $true
                    break
        ...
        Write-Host "        Bağımlılıklar eşzamanlı olarak indiriliyor..." -ForegroundColor $Tema.Uyari

        # NOT: Job'lar artık tek bir $jobs dizisine değil, ayrı isimli değişkenlere
        # atanıyor. Öncesinde $jobs | Receive-Job ile toplanan dönüş değerleri hiç
        # okunmuyordu; onun yerine Test-Path $appinst / $vclibs "indirme başarılı
        # mıydı" sorusuna vekil olarak kullanılıyordu. Ama Invoke-WebRequest -OutFile
        # bağlantı 3 denemede de başarısız olsa bile diskte yarım/bozuk bir dosya
        # bırakabiliyor; bu durumda Test-Path yine $true dönüp bozuk dosyayı
        # Add-AppxPackage'a veriyordu ve gerçek hata (ağ sorunu) gizleniyordu.
        # Şimdi her job'ın GERÇEK dönüş değeri (True/False) ayrı ayrı okunuyor ve
        # Test-Path ile BİRLİKTE (ikisi de doğru olmalı) başarı kararı veriliyor.
        $vclibsJob = Start-Job -ScriptBlock $indirmeJobScript -ArgumentList "https://aka.ms/Microsoft.VCLibs.$mimari.14.00.Desktop.appx", $vclibs
        $appJob    = Start-Job -ScriptBlock $indirmeJobScript -ArgumentList $appUrl, $appinst
        $licJob    = $null
        if ($licUrl) {
            $licJob = Start-Job -ScriptBlock $indirmeJobScript -ArgumentList $licUrl, $license
        }

        $jobs = @($vclibsJob, $appJob)
        if ($licJob) { $jobs += $licJob }

        # 3. Ortak Animasyonlu Bekleme
        $spinner = @("⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏")
        $c = 0
        while (($jobs | Where-Object State -eq 'Running')) {
            $karakter = $spinner[$c % $spinner.Count]
            Write-Host "`r        $karakter Dosyalar arka planda indiriliyor... " -NoNewline -ForegroundColor $Tema.Uyari
            $c++
            Start-Sleep -Milliseconds 100
        }
        # Animasyon bitince satırı temizle
        ...
        # Her job'ın GERÇEK sonucunu kendi değişkeninden oku (sıra karışıklığı olmaz)
        $vclibsBasarili = [bool](Receive-Job -Job $vclibsJob -Wait -ErrorAction SilentlyContinue)
        $appBasarili    = [bool](Receive-Job -Job $appJob -Wait -ErrorAction SilentlyContinue)
        $licBasarili    = if ($licJob) { [bool](Receive-Job -Job $licJob -Wait -ErrorAction SilentlyContinue) } else { $false }

        $jobs | Remove-Job -Force -ErrorAction SilentlyContinue

        # Başarı kararı artık HEM job'ın gerçekten "başarılı" dönmesine HEM dosyanın
        # var olmasına bağlı — tek başına Test-Path yeterli görülmüyor.
        $vclibsIndirildi = $vclibsBasarili -and (Test-Path $vclibs)
        $appIndirildi    = $appBasarili    -and (Test-Path $appinst)
        $lisansIndirildi = $licBasarili    -and (Test-Path $license)

        # 4. VCLibs Kurulumu (Senkron)
        Write-Host "        VCLibs ($mimari) kuruluyor..." -ForegroundColor $Tema.Soluk
        if ($vclibsIndirildi) {
            ...
            Yaz-Log "VCLibs indirilemedi, kurulum atlandi." 'UYARI'
        }

        # 5. UI.Xaml (Appinst'in içinden manifest okunarak sürüm belirlenir, bu yüzden appinst indikten SONRA yapılmalı)
        ...
        if (-not $zatenVar) {
            ...
            # Bu tekil bir indirme olduğu için ana dosyadaki yeni Indir-Dosya'mızı doğrudan kullanabiliriz
        ...
        # 6. App Installer Kurulumu (Senkron)
        Write-Host "        App Installer kuruluyor..." -ForegroundColor $Tema.Soluk
        $lisansli = $false
        if ($appIndirildi -and $lisansIndirildi) {
        ...
        # ===== Sideload politikasını eski haline döndür =====
        try {
...
if ((-not $env:WT_SESSION) -and ($env:BILGISAYAR_ARACI_WT -ne "1")) {
    ...
    if ($wt) {
        ...
        # NOT: Daha önce bu satır yalnızca "Dosya" dalında ayarlanıyordu; "Komut" (irm|iex)
        # dalında set edilmiyordu. Pratikte wt.exe kendi WT_SESSION'ını atadığı için üstteki
        # kontrol yine döngüyü engelliyordu, ama iki dal arasında asimetri vardı ve bayrak
        # ikinci bir güvenlik katmanı olarak düşünülmüşse her iki yolda da olması gerekir.
...
function Update-AllApps {
    ...
    if (-not $paketler) {
        Write-Host "  ℹ Tüm uygulamalar zaten güncel." -ForegroundColor $Tema.Bilgi
        Write-Result $true "Güncelleme kontrolü tamamlandı."
        Wait-User
        return
    }
    ...
    foreach ($p in $paketler) {
        $sayac++
        $yeniSurum = if ($p.AvailableVersions) { @($p.AvailableVersions)[0] } else { "?" }
    ...
    # Measure-Object ile tekil/dizi fark etmeksizin her zaman kesin sayıyı alırız:
    $toplamSayi = ($secilenler | Measure-Object).Count
    if (-not (Confirm-Islem "$toplamSayi uygulama güncellensin mi?")) {
        Write-Result $false "İşlem iptal edildi."
        Wait-User
        return
    }
    ...
    $basarili  = 0
    $basarisiz = 0
    # Olası bir sıfır durumunda bölme hatasını önlemek için güvenliğe alıyoruz:
    $toplam    = if ($toplamSayi -gt 0) { $toplamSayi } else { 1 }
    $i         = 0

    foreach ($u in $secilenler) {
        $i++
        $yuzde = [int](($i / $toplam) * 100)
        Write-Progress -Activity "Uygulamalar güncelleniyor" -Status "[$i/$toplam] $($u.Ad)" -PercentComplete $yuzde
...
function Start-WindowsUpdate {
    ...
    try {
        ...
        # -Verbose ile her güncellemenin durumu ekrana yansır, sonucu değişkene alıyoruz
        $guncellemeler = Get-WindowsUpdate -AcceptAll -Install -IgnoreReboot -Verbose

        Write-Progress -Activity "Windows Update" -Completed
        Write-Host ""

        # Eğer güncellenecek hiçbir şey yoksa özel mesaj ver
        # NOT: Tek bir güncelleme bulunduğunda $guncellemeler diziye değil tekil
        # nesneye dönebildiği için @() ile diziye zorlayıp .Count'u öyle okuyoruz.
        if (@($guncellemeler).Count -eq 0) {
            Write-Host "  ℹ Sistem zaten güncel." -ForegroundColor $Tema.Bilgi
            Write-Result $true "Güncelleme kontrolü tamamlandı."
        } else {
            Write-Result $true "Windows güncelleme işlemi başarıyla tamamlandı."
        }
    } catch {
        Write-Progress -Activity "Windows Update" -Completed
...
function Clean-Temp {
    ...
    if (-not (Confirm-Islem ("Seçilen " + $secilenKategoriler.Count + " kategori temizlensin mi? (" + ($secilenAdlar -join ", ") + ")"))) {
        Write-Result $false "İşlem iptal edildi."
        Wait-User
        return
    }
...
# ===== KATEGORİ 6: GEREKSİZ WINDOWS OLAY GÜNLÜKLERİ (LOGLAR) =====
    if ($secilenKategoriler -contains 6) {
        try {
            # NOT: "wevtutil el" sistemdeki KAYITLI TÜM günlük kanallarını döndürür;
            # bunların arasında Security (oturum açma/yetki denetim kaydı) ve Setup
            # (kurulum geçmişi) gibi "gereksiz" olmayan, teşhis/denetim değeri taşıyan
            # günlükler de vardır. Security günlüğünü toplu temizlemek ayrıca saldırgan
            # iz-silme teknikleriyle (MITRE ATT&CK T1070.001) örtüştüğü için kurumsal
            # ortamlarda AV/EDR tarafından şüpheli olarak işaretlenebilir. Bu yüzden bu
            # ikisi listeden çıkarılıp yalnızca gerçekten "gereksiz" (Application,
            # System, Operational vb.) günlükler temizleniyor.
            $korunanLoglar = @('Security', 'Setup')
            $loglar = @(wevtutil el 2>$null) | Where-Object { $korunanLoglar -notcontains $_ }
            if ($loglar.Count -gt 0) {
                Write-Host "  ▸ Loglar temizleniyor (Security ve Setup günlükleri korunuyor)..." -ForegroundColor $Tema.Soluk
                $logBasarili = 0
                $logBasarisiz = 0
                foreach ($log in $loglar) {
                    ...
                    try {
                        $proc.Start() | Out-Null
                        # Zaman aşımı süresi büyük günlükler için 3 sn'den 60 sn'ye çıkarıldı.
                        $tamamlandi = $proc.WaitForExit(60000)
...
function Repair-Disk {
    ...
    switch ($anaSecim) {
        ...
        4 {
            Write-Host ""
            Write-Host "  DISM + SFC sırayla çalıştırılıyor..." -ForegroundColor $Tema.Metin
            DISM /Online /Cleanup-Image /RestoreHealth
            sfc /scannow
            Wait-User
        }
...
function Reset-DiskTablosu {
    ...
    try {
        ...
        $hedefBolumler = Get-Partition -DiskNumber $diskNo -ErrorAction Stop
        ...
        # GÜVENLİ VARSAYILAN (fail-safe): Sistem sürücüsü kontrolü herhangi bir nedenle
        # yapılamadıysa (disk çıkarıldı, depolama servisi yanıt vermedi, erişim reddedildi
        # vb.), riske GİRMEYİP işlemi durduruyoruz. Önceki hâlde boş catch bu kritik
        # güvenlik kontrolünü sessizce atlıyordu.
        Write-Host "  ⚠ UYARI: Diskteki bölümler doğrulanamadı." -ForegroundColor $Tema.Hata
        Write-Host ("  Ayrıntı: " + $_.Exception.Message) -ForegroundColor $Tema.UyariSoluk
        Write-Host "  Sistem sürücüsü kontrolü yapılamadığından işlem güvenlik" -ForegroundColor $Tema.Hata
        Write-Host "  nedeniyle iptal edildi." -ForegroundColor $Tema.Hata
        Yaz-Log "Reset-DiskTablosu: Get-Partition kontrolu basarisiz (Disk $diskNo) -> $($_.Exception.Message). Islem guvenlik nedeniyle iptal edildi." 'HATA'
        Wait-User
        return
    }
...
function Backup-Drivers {
    ...
    try {
       ...
       Export-WindowsDriver -Online -Destination $klasor -ErrorAction Stop | ForEach-Object {
            ...
            Write-Host $ad -ForegroundColor $Tema.Metin -NoNewline
...
function App-ExportImport {
    ...
    Write-Host "  1) Yüklü uygulama listesini dışa aktar" -ForegroundColor $Tema.Baslik
    Write-Host "  2) Dosyadan uygulamaları içe aktar (kur)" -ForegroundColor $Tema.Baslik
    Write-Host "  0) Ana menüye dön" -ForegroundColor $Tema.Soluk
    Write-Host ""
        ...
        if ($dosya) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace($icerik)) {
                    $icerikObj = $icerik | ConvertFrom-Json -ErrorAction Stop
                    $gecerli = $true
                }
            } catch {
                $gecerli = $false
            ...
            if ($onay -eq "E" -or $onay -eq "e") {
                ...
                # ===== Hedef paket ID'lerini JSON'dan çıkar =====
                # NOT: Daha önce içe aktarma sonucu winget'in İNGİLİZCE konsol metninden
                # ("already installed" / "Successfully installed") sayılıyordu. Türkçe
                # Windows'ta winget çıktısı Türkçe olduğu için bu arama hiç eşleşmiyor ve
                # özet her zaman "0 / 0" gösteriyordu — işlem aslında başarılı olsa bile.
                # Bunun yerine dosyadaki hedef listesine karşı, içe aktarmadan ÖNCE ve
                # SONRA gerçekten kurulu olan paketleri (Get-WinGetPackage üzerinden,
                # dilden bağımsız) karşılaştırıyoruz — script'in geri kalanıyla aynı yaklaşım.
                $hedefIdler = @()
                try {
                    foreach ($kaynak in $icerikObj.Sources) {
                        foreach ($paket in $kaynak.Packages) {
                            if ($paket.PackageIdentifier) { $hedefIdler += "$($paket.PackageIdentifier)" }
                        }
                    }
                } catch {}
                $hedefIdler = @($hedefIdler | Select-Object -Unique)

                # NOT: Burada sadece modülün VAR OLUP OLMADIĞINA bakılıp yoksa sessizce
                # detaysız bir özete düşülüyordu. Update-AllApps ise aynı modül eksikse
                # önce kurmayı dener (Assert-WinGetModulu). Tutarlılık için burada da
                # aynı yol izleniyor; kurulum başarısız olursa yine dilden bağımsız
                # sayım yapılamaz ama en azından bir kez denenmiş olur.
                $modulVar = Assert-WinGetModulu
                if ($modulVar) { Import-Module Microsoft.WinGet.Client -ErrorAction SilentlyContinue }

                function Get-KuruluIdSeti {
                    if (-not $modulVar) { return $null }
                    try { return @(Get-WinGetPackage -ErrorAction Stop | ForEach-Object { $_.Id }) }
                    catch { return $null }
                }

                $onceSet = Get-KuruluIdSeti
                ...
                $sonraSet = Get-KuruluIdSeti

                if ($hedefIdler.Count -gt 0 -and $null -ne $onceSet -and $null -ne $sonraSet) {
                    $zatenKurulu = @($hedefIdler | Where-Object { $onceSet -contains $_ }).Count
                    $yeniKurulan = @($hedefIdler | Where-Object { ($sonraSet -contains $_) -and ($onceSet -notcontains $_) }).Count
                    $basarisiz   = @($hedefIdler | Where-Object { $sonraSet -notcontains $_ }).Count
                    $toplam      = $hedefIdler.Count
                    ...
                    if ($basarisiz -gt 0) {
                        Show-Line ("  Kurulamayan       : " + $basarisiz + " uygulama") $Tema.Hata
                    }
                    Show-Divider
                    Show-Line ("  Hedef toplam      : " + $toplam + " uygulama") $Tema.Baslik
                    Show-Bottom
                    Write-Host ""

                    if ($kod -eq 0 -and $basarisiz -eq 0) {
                        if ($yeniKurulan -gt 0) {
                        ...
                        Write-Result $false "İçe aktarma tamamlandı ancak $basarisiz uygulama kurulamadı (Kod: $kod)."
                    }
                } else {
                    # Modül yoksa dil-bağımsız sayım yapılamaz; sadece exit code'a göre bilgilendir
                    if ($kod -eq 0) {
                        Write-Result $true "İçe aktarma tamamlandı."
                    } else {
                        Write-Result $false "İçe aktarma tamamlandı ancak bazı uygulamalar kurulamamış olabilir (Kod: $kod)."
                    }
                    Write-Host "  Not: Ayrıntılı sayım için Microsoft.WinGet.Client PowerShell modülü gereklidir; kurulamadığı için sayım yapılamadı." -ForegroundColor $Tema.Soluk
                }
            } else {
                Write-Result $false "İşlem iptal edildi."
            }
...
function App-Uninstall {
    ...
    try {
        ...
        # NOT: Winget "Kod: 0" ile kaldırmanın başarılı olduğunu bildirse bile,
        # Program Ekle/Kaldır (ARP) kaydı ve winget'in kendi paket önbelleği
        # her zaman anında güncellenmiyor. Bu durumda tek seferlik 1 saniyelik
        # bekleme yanlışlıkla "hâlâ yüklü" sonucu verip başarılı bir kaldırmayı
        # hatalı gösterebiliyordu. winget zaten başarı bildirmişse, birkaç kez
        # daha kısa aralıklarla tekrar kontrol ederek önbellek gecikmesine
        # tolerans tanıyoruz.
        if ($kod -eq 0 -and $halaVar) {
            $tekrarSayisi = 0
            while ($halaVar -and $tekrarSayisi -lt 3) {
                Start-Sleep -Seconds 2
                $halaVar = Test-WingetPaketYuklu -HedefIdVeyaAd $hedef
                $tekrarSayisi++
            }
