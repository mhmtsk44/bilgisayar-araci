...
function Test-LTSC {
    ...
    return [bool]($kural1 -or $kural2 -or $kural3)
}
...
# İlk yol (PSGallery / App Installer) tutmazsa devreye giren dayanıklı yedek.
function Install-WingetManuel {
    ...
    # App Installer (lisansli - provisioned, zaman asimli)
    ...
    # Lisansli yol tutmazsa: dogrudan aka.ms/getwinget (zaman asimli)
    if (-not $lisansli) {
...
function Install-Winget {
    Write-Host "Winget durumu kontrol ediliyor..." -ForegroundColor Cyan
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        Write-Host "Winget bu sistemde zaten kurulu!" -ForegroundColor Green
        return $true
    }
    ...
    if ($ltsc) {
        # ---------------------------------------------------------
        # LTSC SİSTEM SENARYOSU -> once PSGallery
        # ---------------------------------------------------------
        ...
        try {
            ...
            Write-Host "LTSC kurulumu sirasinda beklenmeyen bir hata olustu." -ForegroundColor Red
        }

    } else {
        # ---------------------------------------------------------
        # NORMAL SİSTEM SENARYOSU -> once App Installer
        # ---------------------------------------------------------
        ...
        try {
            ...
            Invoke-WebRequest -Uri "https://aka.ms/getwinget" -OutFile $getwinget -UseBasicParsing -TimeoutSec 120 -ErrorAction Stop
            $ProgressPreference = $eski

            Add-AppxPackage -Path $getwinget -ErrorAction Stop
            Remove-Item $getwinget -Force -ErrorAction SilentlyContinue
        } catch {
            Write-Host "Standart kurulum paketi indirilemedi veya kurulamadi." -ForegroundColor Red
        }
    }

    # ===================== ARA KONTROL: Birincil yol tuttu mu? =====================
    ...
    # ===================== YEDEK YOL 2: Manuel bagimlilik kurulumu =====================
    ...
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        ...
        Write-Host "İşlem Başarısız: Winget kurulamadı. Sistemi yeniden başlatıp tekrar deneyebilirsiniz." -ForegroundColor Red
        return $false
    }
}
...
# Mantık sırası:
#   1-4) Temel güvenlik (boş, kök sürücü, çok kısa) → reddet
#   5) WHITELIST ÖNCE: bilinen-güvenli geçici klasörler (Temp/Prefetch) → HEMEN izin ver
#   6) BLACKLIST: sistem klasörleri + üst klasörleri → reddet
#   7) Varsayılan → reddet (whitelist mantığı)
# Bu sıra sayesinde C:\Windows\Prefetch, C:\Windows yasağına takılmadan temizlenebilir.

# ===================== GÜVENLİK: KORUNAN / İZİNLİ YOLLAR =====================
...
function Test-GuvenliYol {
    ...
    # 1) Boş / geçersiz yol → GÜVENSİZ
    if ([string]::IsNullOrWhiteSpace($Yol)) { return $false }

    # 2) Yolu normalize et (tam yol + sondaki \ temizle + küçük harf)
    try {
    ...
    # 3) Çok kısa yol (örn. "c:") → GÜVENSİZ
    if ($tam.Length -lt 8) { return $false }

    # 4) Kök sürücü mü? (örn. "c:") → GÜVENSİZ
    if ($tam -match '^[a-z]:$') { return $false }

    # 5) İZİN LİSTESİ (WHITELIST) — blacklist'ten ÖNCE kontrol edilir.
    #    Böylece C:\Windows\Temp ve C:\Windows\Prefetch geçer,
    #    ama C:\Windows'un kendisi 6. adımda bloklanır.
    $izinliDesenler = @(
        '\\temp$',        # ...\Temp
        '\\prefetch$',    # C:\Windows\Prefetch
        '\\explorer$',    # ...\Microsoft\Windows\Explorer (thumbnail cache)
        '\\inetcache',    # ...\INetCache
        '\\cache$',       # ...\Cache
        '\\recent$'       # ...\Microsoft\Windows\Recent (son kullanılanlar)
    )
    foreach ($desen in $izinliDesenler) {
        if ($tam -imatch $desen) { return $true }
    }

    # 6) KARA LİSTE (BLACKLIST): tam eşleşme veya yasaklı bir yolun ÜST klasörü
    ...
    # 7) Hiçbir izinli desene uymuyorsa → varsayılan GÜVENSİZ
    return $false
}
...
function Clean-Temp {
    ...
    # İsimli hedefler (Kod 2'den) + Explorer thumbnail (Kod 1'den) + Recent (Kod 2'den)
    $hedefler = @(
    ...
    foreach ($k in $hedefler) {
        ...
        # Boşalan alt klasörleri temizle
        ...
        # Her hedef için ayrı geri bildirim (Kod 2'den)
...
#  ---------------------------------------------------------------------------------
#  ✅ Güvenli disk seçimi ([int]::TryParse)
#  ✅ Çok katmanlı güvenlik (USB re-check + >512 GB uyarısı + disk adı onayı)
#  ✅ Salt-okuma listeleme seçeneği (risksiz inceleme)
#  ✅ Akıllı & güvenli etiket (eski etiket → disk adı → "USB", karakter temizliği)
#  ✅ Sürücü harfi doğrulaması (Madde 3)
#  ✅ Sağlam autorun savunması (attrib + try/catch, ReadOnly yok) (Madde 1)
#  ✅ Düzeltilmiş ACL kök koruması (AD,WD → sürücü erişilebilir kalır)
#  NOT: Admin kontrolü YOK — ana betik zaten Yönetici olarak çalışıyor.
# ==================================================================================
function Protect-USB {
    ...
    # ---------- Disk Seçimi (Güvenli Sayı Dönüşümü) ----------
    ...
    switch ($islemTipi) {
        ...
        # ================= SEÇENEK 1: GÜVENLİ HALE GETİR + BİÇİMLENDİR =================
        "1" {
            Write-Host ""
            ...
            try {
                ...
                # ---------- AKILLI & GÜVENLİ ETİKET (disk adından türetme) ----------
                # Öncelik: 1) Eski birim etiketi  2) Disk adı (FriendlyName)  3) "USB"
                ...
                # NTFS etiketinde geçersiz karakterleri temizle ( \ / : * ? " < > | )
                $eskiEtiket = ($eskiEtiket -replace '[\\/:*?"<>|]', '').Trim()
                if ([string]::IsNullOrWhiteSpace($eskiEtiket)) { $eskiEtiket = "USB" }
                # NTFS etiket sınırı: 32 karakter
                if ($eskiEtiket.Length -gt 32) { $eskiEtiket = $eskiEtiket.Substring(0, 32).Trim() }

                # ---------- Diski Temizle + NTFS Biçimlendir ----------
                ...
                # ---------- SÜRÜCÜ HARFİ DOĞRULAMASI (Madde 3) ----------
                # -AssignDriveLetter bazen harfi hemen atamaz; kısa bir bekleme + yeniden okuma.
                ...
                # ---------- SAĞLAM AUTORUN SAVUNMASI (Madde 1) ----------
                # autorun.inf'i KLASÖR olarak oluşturmak, kötü amaçlı yazılımın
                # kendi autorun.inf DOSYASINI oluşturmasını engeller.
                # attrib kullanıyoruz (Set-ItemProperty klasörde kırılgan); ReadOnly YOK.
                # try/catch: autorun koruması başarısız olsa bile biçimlendirme geçerli kalır.
                $autorunYolu = "$harf\autorun.inf"
                try {
                    New-Item -Path $autorunYolu -ItemType Directory -Force -ErrorAction Stop | Out-Null
                    attrib +h +s $autorunYolu                                   # +h Gizli, +s Sistem
                    icacls $autorunYolu /deny "*S-1-1-0:(OI)(CI)(F)" /Q | Out-Null   # EN SON erişim reddi
                } catch {
                    Write-Host ("  ⚠ Autorun koruması uygulanamadı: " + $_.Exception.Message) -ForegroundColor $Tema.Hata
                }

                # ---------- Kök Dizine SADECE OLUŞTURMA Yasağı (düzeltilmiş ACL) ----------
                #   WD = yeni DOSYA oluşturma,  AD = yeni KLASÖR oluşturma
                #   Miras (OI)(CI) YOK → alt klasörler (GüvenliDosya) etkilenmez, sürücü erişilebilir kalır.
                icacls "$harf\" /deny "*S-1-1-0:(AD,WD)" /Q | Out-Null

                # ---------- GüvenliDosya'ya Tam Yetki ----------
        ...
        # ================= SEÇENEK 2: BÖLÜMLERİ LİSTELE (SALT OKUMA) =================
        "2" {
            Write-Host ""
...
function Repair-Disk {
    ...
    # GÜVENLİ DÖNÜŞÜM: harf girilirse çökmez
    [int]$anaSecim = 0
    ...
    switch ($anaSecim) {
        ...
        3 {
            # ===== DİSK SEÇMELİ CHKDSK (Kod 2'nin gövdesi) =====
...
function Invoke-ChkdskSecmeli {
    ...
    # ============================================================
    #  SEÇİM DÖNGÜSÜ: yanlış seçersen geri dönüp tekrar seçebilirsin
    # ============================================================
    $secilen = $null
    while (-not $secilen) {

        # --- Diskleri ve bölümleri listele (her turda yeniden çiz) ---
        $harfListesi = @()
        $sayac = 0
        ...
        if (-not [int]::TryParse($girdiSecim, [ref]$secim)) {
            ...
            continue   # ← döngü başına dön, listeyi tekrar göster
        }
        if ($secim -eq 0) {
            Write-Result $false "İşlem iptal edildi."
            Write-Host ""; Read-Host "  Devam etmek için Enter'a basın"; return
        ...
        if (-not $aday) {
            ...
            Write-Host ""
            continue   # ← yanlış numara → tekrar seç
        }

        # --- SEÇİLENİN KİMLİK SATIRI (DİSK MODELİ + ETİKET) ---
        # Model her zaman gösterilir; etiket varsa köşeli parantezle eklenir.
        ...
        $secilen = $aday   # ← onaylandı, döngüden çık
    }

    # ============================================================
    #  Seçim kesinleşti → $secilen ve $secimAdi hazır
    # ============================================================
    $harf = $secilen.Harf
    $fs   = $secilen.FS
...
function App-ExportImport {
    ...
    if ($sec -eq "1") {
        ...
        if ($dosya) {
            ...
            # >>> YENİ: JSON geçerlilik ön kontrolü
            $gecerli = $false
            try {
